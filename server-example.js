...
app.post('/api/data', async (req, res) => {
  try {
    ...
    switch (operation) {
      ...
      // Payments
      case 'addPayment':
        const payment = await addPaymentOp(data.projectId, data.amount, data.date, data.note)
        res.json(payment)
        break
      ...
      // Expenses
      case 'addExpense':
        const expense = await addExpenseOp(data.projectId, data.amount, data.date, data.description)
        res.json(expense)
        break
      ...
      case 'addBankSpending':
        const bankSpending = await addBankSpendingOp(data.amount, data.date, data.description)
      ...
      case 'addCharitySpending':
        const charitySpending = await addCharitySpendingOp(data.amount, data.date, data.description)
