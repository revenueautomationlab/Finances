...
async function migrate() {
  try {
    ...
    console.log(
      `   - ${dbData.bankSpending?.length || 0} bank spending records`,
    );
    console.log(
      `   - ${dbData.charitySpending?.length || 0} charity spending records\n`,
    );
    ...
    if (dbData.bankSpending && dbData.bankSpending.length > 0) {
      ...
      console.log(
        `✅ Inserted ${dbData.bankSpending.length} bank spending records\n`,
      );
    }
    ...
    if (dbData.charitySpending && dbData.charitySpending.length > 0) {
      ...
      console.log(
        `✅ Inserted ${dbData.charitySpending.length} charity spending records\n`,
      );
    }
